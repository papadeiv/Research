...
\begin{abstract}
        Combining recent results in nonautonomous dynamics and statistical estimation for stochastic differential equations we derive a fully data-driven pipeline for the accurate forecast of tipping times in complex systems. \absbreak 
\end{abstract}
...
It follows trivially that the tipping time of the pullback attractor is given by $\tau_{\mathrm{pb }}\approx \tau + 2.338\,\varepsilon^{-1/3}$ and thus the opportunity window scales with the reciprocal of the cubic root of the parameter speed, i.e. $\Delta\tau\propto\varepsilon^{-1/3}$.
...
\end{equation}
%
Let us consider time instances $t\ll\tau$ away from the saddle-node bifurcation; in this case $f(x_{\mathrm{pb}},\mu)\approx f\bigl(x_+(\mu),\mu\bigr)$ which implies $\partial_x f(x_{\mathrm{pb}},\mu)\approx\lambda(t)$.
...
\end{equation}
%
Provided a sample $\mathcal{X} = \{X_j\}_{j=1,\dots,n}$ in a time window $T_w=n\Delta$, and a choice of the model function $f$, Eq.~\eqref{eq:increments} can be interpreted as a ordinary least square regression problem, with the residuals $r_j\coloneqq Y_{j+1} - f(X_j)=\tilde{\xi}_t$ being normally distributed.
This technique is part of a broader class of drift inference methods called \emph{quasi-maximum likelihood estimators} \cite{Jimenez2020}.
...
The application of quasi-maximum likelihood estimations for the construction of early-warning signals is much more recent than the more established autoregressive and Ornstein-Uhlenbeck approximation, with early adoptions found in \cite{Papapicco2026}.
...
\end{equation}
%
where $\eta(t)=\frac{1}{4\sqrt{\varepsilon(\tau-t)^3}}$ quantifies the lag correction of the return rate of the pullback attractor.
...
\end{figure}
%
The critical treshold \eqref{eq:admissible_time_horizon} gives us a criterion for which the estimation of the frozen return rate yields admissible results for the extrapolation of the tipping time, as the lag correction $\eta(t)$ of the pullback attractor with respect to the quasi-steady equilibrium $x_+(\mu(t))$ is negligible. 

\subsubsection{Finite-sample effects}\label{subsubsec:effects}
In our data-driven pipeline, the estimation of the return rate is performed out of a sample $\mathcal{X}$ that is assumed to be a solution of \eqref{eq:model}. 
Provided that the sample is in the admissible time horizon $t<T_c$ quantified above, a local estimation of $\alpha(t)$ requires that the sample $\mathcal{X}$ is recorded over a window of time in which the leading eigenvalue $\lambda(t)$ at the quasi-steady equilibrium remains relatively unchanged. 
...
We shall henceforth adopt an ensemble approach in which $\Omega = \{\mathcal{X}_{j}\}_{j=1,\dots,N}$ denotes a collection of $N>1$ independent and identically distributed finite samples that correspond to different noise realizations of \eqref{eq:model}.
...
We can thus identify the bias $b_{\hat\beta}$ and uncertainty $u_{\hat\beta}$ of the estimator $\hat\beta$ to be 
%
...
\end{equation}
%
Depending on the context, the bias and uncertanity of statistical estimators are also referred to as accuracy and precision respectively; here these two terminologies will be used interchangeably.
In the following we shall assume each sample $\mathcal{X}_j$ in the ensemble $\Omega$ to be a sequence ofstationary residuals obtained by detrending a finite windowed solution of \eqref{eq:model} that is collected sufficiently away from the saddle-node bifurcation.
The resulting Ornstein-Uhlenbeck process \eqref{eq:oup} can be approximated by an autoregressive model of order one \eqref{eq:autoregressive}.
...
Following \cite{Yu2012}, and using \eqref{eq:oup_variance}, a second-order Taylor expansion yields
%
...
\end{equation}
%
Plugging \eqref{eq:bias_law} into \eqref{eq:taylor_expansion} yields a closed form expression for the bias in the estimation of the leading eigenvalue of the sample $\mathcal{X}$ using the variance of its residuals and within the Ornstein-Uhlenbeck assumption
%
...
\end{equation*}
%
Using the above, and following the same derivation for the sample variance, a second-order Taylor expansion yields \cite[Eq.~$(12)$]{Yu2012}
%
...
The return rate estimator \eqref{eq:LLS} is the square the ordinary least squares solution of the Euler-Maruyama approximation \eqref{eq:normal_equation}, which coincides with the estimated leading eigenvalue of the sample $\mathcal{X}$ by definition.
Its bias is thus more straightfoward to compute \cite[Eq.~$(4.5)$]{Florens1989} and it reads
%
\begin{equation}\label{eq:LLS_bias}
  b_{\hat\lambda}^{\text{\tiny OLS}} = \frac{\Delta\lambda^2}{2} . 
\end{equation}
%
Notice that, while \eqref{eq:OUP_bias} and \eqref{eq:AC1_bias} are negative and size dependent (i.e. they both vanish in the large sample limit $n\to\infty$), the bias in the least squares estimator \eqref{eq:LLS_bias} is positive and size independent.
This property is particularly important as, depending on applications, modellers might want to prefer one estimator over the other.
We remark that the ordinary least square estimator becomes particularly valuable for applications in which data is scarce (albeit regularly and finely sampled) and thus the typical size of the available sample $\mathcal{X}$ is small.

\paragraph{Accuracy and precision of the estimators}
Eqs.~\eqref{eq:OUP_bias}-\eqref{eq:LLS_bias} measure the accuracy for the estimation of the leading egeinvalue $\lambda$ of a sample. To convert those into biases of the return rate $\alpha$, notice that
%
\begin{equation}
