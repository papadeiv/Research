...
First notice that the return rate estimator using the Ornstein-Uhlenbeck approximation \eqref{eq:OUP} requires the knowledge of the noise intensity $\sigma$ other than the sample variance $\hat\gamma$.
Since $\sigma$ is usually unknown, one popular work around this limitation is to estimate $\lambda$ via maximum likelihood estimation \cite{Sahalia2002}.
Away from the bifurcation $|\lambda|\gg0$, the bias of this estimator was derived in \cite[Eq.~$(12)$]{Yu2012} and it reads
%
\begin{equation}\label{eq:yu_bias}
  b_{\hat\lambda} = - \frac{3 + \rho^{-2}}{2\Delta(n-1)} ,
\end{equation}
%
where $\rho$ is the autocorrelation at lag-$1$ given in \eqref{eq:ac1}.
Its estimation from a finite sample \eqref{eq:ac1_sample} is also biased as quantified in \cite[Eq.~$(21)$]{Kendall1954}
%
\begin{equation}\label{eq:kendall_bias}
  b_{\hat\rho} = - \frac{1 + 3\rho}{n-1} .
\end{equation}
%
On the other hand, the ordinary least squares solution of the Euler-Maruyama approximation \eqref{eq:normal_equation} is positively biased as derived in \cite[Eq.~$(4.5)$]{Florens1989}
%
\begin{equation}\label{eq:florens_bias}
  b_{\hat\theta} = \frac{\Delta\lambda^2}{2} . 
\end{equation}
%
Notice that while \eqref{eq:yu_bias} and \eqref{eq:kendall_bias} vanish in the large sample limit $n\to\infty$ the accuracy of the ensemble mean estimator of the quasi-maximum likelihood \eqref{eq:florens_bias} only depends on the sampling frequency $\Delta$.
...
To convert \eqref{eq:yu_bias}-\eqref{eq:florens_bias} into measures of accuracy for the estimators of the return rate $\alpha$, notice that
%
\begin{equation}
  \E[\hat\alpha] = \E[\hat\lambda^2] = \E[\hat\lambda]^2 + \Var[\hat\lambda] = (b_{\hat\lambda} + \lambda)^2 + u^2_{\hat\lambda} ,
\end{equation}
%
where in the last equality we used the definitions \eqref{eq:bias_and_uncertainty} and the fact that $\E[\hat\beta-\beta] = \E[\hat\beta] - \beta$.
Expanding the quadratic term and using the definition of the (analytical) return rate $\alpha$ we obtain
%
