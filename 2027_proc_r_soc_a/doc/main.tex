...
\end{equation}
%
The use of the sample variance as an early-warning signal dates back to the very first works in the topic and it has since been established as a benchmark precursor of critical slowing down for the detection of bifurcation-induced critical transitions (see e.g. \cite{Kuehn2011} and references therein).
...
Following a standard delta method \cite[\S~$5.5.4$]{Casella2024}, and using \eqref{eq:oup_variance}, we obtain a second-order approximation of the bias in the estimated leading eigenvalue within the Ornstein-Uhlenbeck assumption, which reads
%
...
\end{equation}
%
Plugging \eqref{eq:bias_law} into \eqref{eq:taylor_expansion} yields a closed form expression for the bias in the estimation of the leading eigenvalue of the sample $\mathcal{X}$ using the variance of its residuals
%
...
\end{equation}
%
for which \eqref{eq:LLS} is thus an unbiased estimator of the leading eigenvalue.
