...
Given the estimate sequence $\hat\alpha$, we can fit a linear model of the form $\hat\alpha(t) = \hat m t + \hat q$, with slope $\hat m$ and offset $\hat q$. 
...
Conseqentially, we translated the problem of forecasting the breaking time $\tau$ from timeseries data $\mathcal{X}$ into a trivial, linear extrapolation $\hat\tau$ from estimates $\hat\alpha$.
...
\end{equation}
%
Depending on the context, the bias and uncertainty of statistical estimators are also referred to as accuracy and precision respectively; here these two terminologies will be used interchangeably.
In the following we shall assume each sample $\mathcal{X}_j$ in the ensemble $\Omega$ to be a sequence of stationary residuals obtained by detrending a finite windowed solution of \eqref{eq:model} that is collected sufficiently away from the saddle-node bifurcation.
